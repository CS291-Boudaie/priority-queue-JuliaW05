...
class MinHeap:
    ...
    def add(self, priority, item):
        ...
        self._bubble_up(self.__len__() - 1)
    ...
    def _bubble_down(self, idx):
        ...
        while (left_index < self.__len__()):

            if left_index < self.__len__() and right_index < self.__len__():
                if self.data[left_index][0] < self.data[right_index][0]:
                    if node[0] > self.data[left_index][0]:
                        self.data[index] = self.data[left_index]
                        index = left_index
                    elif node[0] > self.data[right_index][0]:
                        self.data[index] = self.data[right_index]
                        index = right_index
                    else:
                        break
                else:
                    if node[0] > self.data[right_index][0]:
                        self.data[index] = self.data[right_index]
                        index = right_index
                    elif node[0] > self.data[left_index][0]:
                        self.data[index] = self.data[left_index]
                        index = right_index
                    else:
                        break

                if node[0] > self.data[smaller_child][0]:
                    self.data[index] = self.data[left_index]
                    index = left_index
                elif (self.data[right_index][0] < self.data[left_index][0] and node[0] > self.data[right_index][0]):
                    self.data[index] = self.data[right_index]
                    index = right_index
                else:
                    break
