...
class MinHeap:
    ...
    def _bubble_up(self, idx):
        ...
        while (index > 0 and self.data[index][0] < self.data[parent_index][0]):
    ...
    def _bubble_down(self, idx):
        ...
        while (index < self.__len__() and (left_index < self.__len__() or right_index < self.__len__())):

            if left_index < self.__len__() and right_index < self.__len__():

                if self.data[left_index][0] < self.data[right_index][0] and self.data[index][0] > self.data[left_index][0]:
                    self.data[index] = self.data[left_index]
                    index = left_index
                elif (self.data[right_index][0] < self.data[left_index][0] and self.data[index][0] > self.data[right_index][0]):
            ...
            elif left_index > self.__len__() and right_index < self.__len__():
                if self.data[index][0] > self.data[right_index][0]:
                    self.data[index] = self.data[right_index]
                    index = right_index
                right_index = (2 * index + 2)
                
            else:
                if self.data[index][0] > self.data[left_index][0]:
                    self.data[index] = self.data[left_index]
                    index = left_index
                left_index = (2 * index + 1)
